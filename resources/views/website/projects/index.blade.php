                                ...
                                <div class="card h-100 border-0 shadow-sm">
                                    ...
                                    <div class="position-relative">
                                        <img src="{{ asset('storage/' . $project->image) ?? asset('assets/img/contours.jpg') }}"
                                            class="card-img-top thumbnail" alt="{{ $project->title }}"
                                            style="height: 250px; object-fit: cover;">

                                        @if ($project->status === 'completed')
                                            <span
                                                class="badge bg-success position-absolute top-0 end-0 m-2">Completed</span>
                                        @endif

                                        @if ($project->status === 'ongoing')
                                            <span
                                                class="badge bg-warning position-absolute top-0 end-0 m-2">Ongoing</span>
                                        @endif
                                    </div>
    ...
    @truncate($project->description)
                                    ...
                                    <div class="card-footer bg-white border-top-0 pt-0 pb-3">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <a href="/projects/{{ $project->id }}/show" class="btn btn-sm btn-outline-success px-3">Project in
                                                detail</a>
